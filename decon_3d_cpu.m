function decon_3d_cpu(xres, yres, zres, background, iteration, filename)
...
if isempty(header.channels)
    ...
    if (channels ~= size(xres, 2)) || (channels ~= size(yres, 2)) || (channels ~= size(zres, 2)) || (channels ~= size(background, 2)) || (channels ~= size(iteration, 2))
        error("Channel number of image stack is inconsistent with xres, yres, zres, background or iteration array size.");
    end
end
