function output_image = decon_core(input_image, FWHM, iterations)
% Updated decon_core 06/15/2023 Xuesong Li
...
kernal_size = 2 * ceil(2 * sigma) + 1;  % kernal_size = 9
%{
...
if nz == 1
    ...
    % 3D stack
    Estimate = padarray(input_image, [npad(2), npad(1), npad(3)], 'replicate', 'both');
end
[ny_pad, nx_pad, nz_pad] = size(Estimate);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CPU or GPU? %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
g = gpuDevice(1); reset(g);
if (ny_pad * nx_pad * nz_pad * 4) <= (g.FreeMemory / 6)
    disp(['GPU Memory before RL deconvolution: ', num2str(g.FreeMemory / 1024 / 1024 / 1024), ' GB']);
    gpu_Estimate = gpuArray(Estimate);
    gpu_flag = 1;
else
    disp('RL deconvolution using CPU.');
    gpu_flag = 0;
end
